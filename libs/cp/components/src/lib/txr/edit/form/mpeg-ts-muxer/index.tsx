...
import {ETxrApplicationType, ETxrMuxer, ETxrOutputType} from "@nxt-ui/cp/types";
...
export const MpegTsMuxer: FC = () => {
    ...
    const values = useSelector(txrEditSelectors.mpegTsMuxer.values);
    const audioEncoders = useSelector(txrEditSelectors.audioEncoder.values);
    ...
    const errors = useSelector(txrEditSelectors.mpegTsMuxer.errors);
    const changeMuxerHandler = useCallback(
        (e: SelectChangeEvent<unknown>) => {
            dispatch(txrEditActions.setMuxer(e.target.value as ETxrMuxer));
        },
        [dispatch]
    );
    const changeMuxrateHandler = useCallback(
        (e) => {
            dispatch(txrEditActions.setMuxrate(e.target.value));
        },
        [dispatch]
    ...
    const changeServiceNameHandler = useCallback(
        (e) => {
            dispatch(txrEditActions.setServiceName(e.target.value));
        },
        [dispatch]
    ) as ChangeEventHandler<HTMLInputElement>;
    ...
    const changeServiceProviderHandler = useCallback(
        (e) => {
            dispatch(txrEditActions.setServiceProvider(e.target.value));
        },
        [dispatch]
    ) as ChangeEventHandler<HTMLInputElement>;
    ...
    const changeProgramNumberHandler = useCallback(
        (e) => {
            ...
            if (value || (typeof value === "number" && !isNaN(value))) {
                dispatch(txrEditActions.setProgramNumber(value));
            } else {
                dispatch(txrEditActions.setProgramNumber(null));
            }
        },
        [dispatch]
    ) as ChangeEventHandler<HTMLInputElement>;
    ...
    const changeVideoPidHandler = useCallback(
        (e) => {
            const value = parseInt(e.target.value);
            dispatch(txrEditActions.setVideoPid(value));
        },
        [dispatch]
    ) as ChangeEventHandler<HTMLInputElement>;
    ...
    const changeAudioPidHandler = useCallback(
        (index: number) => (e) => {
            const value = parseInt(e.target.value);
            dispatch(txrEditActions.setAudioPid({index, value}));
        },
        [dispatch]
    ...
    const changePmtPidHandler = useCallback(
        (e) => {
            const value = parseInt(e.target.value);
            if (value || (typeof value === "number" && !isNaN(value))) {
                dispatch(txrEditActions.setPmtPid(value));
            } else {
                dispatch(txrEditActions.setPmtPid(null));
            }
        },
        [dispatch]
    ) as ChangeEventHandler<HTMLInputElement>;
    ...
    const changePmtPeriodHandler = useCallback(
        (e) => {
            ...
            if (typeof value === "number" && !isNaN(value)) {
                dispatch(txrEditActions.setPmtPeriod(value));
            } else {
                dispatch(txrEditActions.setPmtPeriod(null));
            }
        },
        [dispatch]
    ) as ChangeEventHandler<HTMLInputElement>;
    ...
    const changePcrPidHandler = useCallback(
        (e) => {
            const value = parseInt(e.target.value);
            if (typeof value === "number" && !isNaN(value)) {
                dispatch(txrEditActions.setPcrPid(value));
            } else {
                dispatch(txrEditActions.setPcrPid(null));
            }
        },
        [dispatch]
    ) as ChangeEventHandler<HTMLInputElement>;
    ...
    const changePcrPeriodHandler = useCallback(
        (e) => {
            const value = parseInt(e.target.value);
            if (typeof value === "number" && !isNaN(value)) {
                dispatch(txrEditActions.setPcrPeriod(value));
            } else {
                dispatch(txrEditActions.setPcrPeriod(null));
            }
        },
        [dispatch]
    ) as ChangeEventHandler<HTMLInputElement>;
    ...
    const changeTsIdHandler = useCallback(
        (e) => {
            ...
            if (value && !isNaN(value)) {
                dispatch(txrEditActions.setTsId(value));
            } else {
                dispatch(txrEditActions.setTsId(null));
            }
        },
        [dispatch]
    ) as ChangeEventHandler<HTMLInputElement>;
    ...
    const changeAddScteHandler = useCallback(
        (e) => {
            dispatch(txrEditActions.setAddScte(e.target.value));
        },
        [dispatch]
    ) as ChangeEventHandler<HTMLInputElement>;
    ...
    const muxerValues = useMemo(() => {
        const result = [ETxrMuxer.libmpegts];
        if (applicationType === ETxrApplicationType.AVDS2) {
            result.push(ETxrMuxer.mainconcept);
        }
        return result;
    }, [applicationType]);
